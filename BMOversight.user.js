...
// ============================================================
// SECTION 1: SERVER MONITOR & ALERT SYSTEM
// Activates on: /servers/* pages
// (Original: BMserver.user.js v1.0.0)
// ============================================================
(function () {
    'use strict';
    ...
    // Alt detection: ignore events during first 20 min after page load (cold-start guard)
    const _scriptStartTime   = Date.now();
    const COLD_START_WAIT_MS = 20 * 60 * 1000;
    ...
    class DebugConsole {
        ...
        getLogsAsText() {
            const header = `BM Oversight v${this.version} Debug Logs
...
`;
    ...
    // Log script startup
    debugConsole.info(`BM Oversight v${SCRIPT_VERSION} loaded`, {
        url: window.location.href,
        userAgent: navigator.userAgent,
    ...
    class ServerMonitor {
        ...
        updatePopulationDisplay() {
            ...
            // Unique players in last 24h
            const _now = Date.now();
            const _cut24h = _now - 86400000;
            const _ids24h = new Set();
            for (const e of this.activityLog) {
                if (!e.playerId) continue;
                if (e.timestamp >= _cut24h) _ids24h.add(e.playerId);
            }
            const u24h = _ids24h.size;
                ...
                <div style="background: rgba(111, 66, 193, 0.1); padding: 10px; border-radius: 5px; margin-bottom: 10px;">
                    ...
                    <div style="border-top: 1px solid rgba(111,66,193,0.3); padding-top: 7px;">
                        <div style="display: flex; justify-content: space-between; align-items: center;">
                            <div style="color: #6f42c1; font-size: 11px; font-weight: bold;">Unique Players (24h)</div>
                            <div style="background: rgba(255,255,255,0.05); border-radius: 3px; padding: 4px 8px; text-align: center;">
                                <div style="color: white; font-size: 14px; font-weight: bold;">${u24h}</div>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        logActivity(playerName, playerId, action) {
            ...
            // Cold-start guard: skip alt scans for the first 20 min to let history accumulate.
            if ((action === 'joined' || action === 'left') && (Date.now() - _scriptStartTime >= COLD_START_WAIT_MS)) {
        ...
        const renderTab = (tab) => {
            ...
            if (tab === 'Session History') {
                ...
                if (alts.length === 0) {
                    ...
                    let html = `<div style="font-size:10px;color:#adb5bd;margin-bottom:8px;">Relay pattern detected: one account logs off and the other joins within 5 min. They are rarely online simultaneously.</div>`;
                    for (const alt of alts) {
    ...
    //
    // Thresholds: switch window is 5 min (real alt swaps are fast, not slow casual switches).
    ...
    const SWITCH_MAX_MS     =  5 * 60 * 1000; // 5 min — real account switches happen quickly
    const MIN_HANDOFFS      = 5;               // ≥5 total (A→B + B→A) — strong pattern required
    const MIN_HANDOFF_RATE  = 0.40;            // ≥40% of target's leave events
    const MIN_TARGET_LEAVES = 8;               // target must have ≥8 leave events for reliable rate
    ...
    const detectAlts = (targetId) => {
        ...
        // Require enough leave events for rate-based detection to be statistically meaningful
        if (targetLeaveEvts.length < MIN_TARGET_LEAVES) return [];
    ...
    window.showAltsModal = (playerId) => {
        ...
        desc.textContent = `Players who joined within 5 min of ${displayName} leaving (or vice-versa) ≥5 times — potential alt accounts.`;
    ...
    const showWelcomePopup = () => {
        ...
        box.innerHTML = `
            <div style="text-align:center;margin-bottom:20px;">
                ...
                <h2 style="margin:0 0 6px;color:#17a2b8;font-size:20px;">Welcome to BM Oversight</h2>
...
// Activates on: /players/* pages
// (Original: BMplayer.user.js v1.0.0)
// ============================================================
(function () {
    'use strict';
...
`;
    ...
    // Log script startup
    debugLog('info', `BM Oversight v${SCRIPT_VERSION} loaded`, {
        url: window.location.href,
        userAgent: navigator.userAgent,
        autoPullEnabled,
        debugConsoleEnabled
    ...
    // Run an initial update check if allowed, then periodically every 1 minute
    if (checkForUpdates) {
        setTimeout(() => { checkForUpdatesAvailable(); }, 1000);
        setInterval(() => { checkForUpdatesAvailable(); }, 60 * 1000);
    }
